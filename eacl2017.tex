...
\newcite{iacobacci-pilehvar-navigli:2015:ACL-IJCNLP} disambiguate the words in a corpus using a state-of-the-art WSD system and then produce continuous represenations of word senses based on the distributional information obtained from the annotated corpus. %before learning the embeddings. 
...
Instead of using the senses to refine the vector representations as in \cite{faruqui-EtAl:2015:NAACL-HLT}, we rather use them to improve the lexical substitution rankings proposed by the models. % in a task. 
...
& \multicolumn{2}{c}{Syn.VSM} & \multicolumn{2}{c}{AddCos (cwin=1)} \\ \hline
& Unfiltered GAP & Oracle GAP & Unfiltered GAP & Oracle GAP \\ \hline
PPDBClus & 0.528 & 0.661 & 0.533 & 0.656 \\ \cdashline{1-5}[1pt/2pt]
WordNet & 0.528 & 0.655 & 0.533 & 0.651 \\ \hline
Choose-K: \# WN Synsets \footnotesize{(avg)} & 0.528 & 0.639 & 0.533 & 0.636  \\ \cdashline{1-5}[1pt/2pt]
Choose-K: \# WN Synsets \footnotesize{(max, no co-clustering)} & 0.528 & 0.695* & 0.533 & 0.69*  \\ \cdashline{1-5}[1pt/2pt]
Choose-K: \# WN Synsets \footnotesize{(max, co-clustering)} & 0.528 & 0.69** & 0.533 & 0.683  \\ \hline
Choose-K: Optimize NMI  \footnotesize{(avg)} & 0.528 & 0.668 & 0.533 & 0.662 \\ \cdashline{1-5}[1pt/2pt]
Choose-K: Optimize NMI  \footnotesize{(max, no co-clustering)} & 0.528 & 0.719 *** & 0.533 & 0.714 ****  \\ \cdashline{1-5}[1pt/2pt]
Choose K: Optimize NMI \footnotesize{(max, co-clustering)} & 0.528 & 0.718 ***** & 0.533 & 0.71 ******  \\
\end{tabular}
}
  \caption{\footnotesize{GAP scores of the unfiltered and best sense-filtered rankings produced by the Syn.VSM and AddCos models. Best performing configurations are: * Min PPDB Score 2.0, cluster PP's only, use PP-PP mask; ** Min PPDB Score 2.0, use PP-PP mask only; *** Min PPDB Score 2.0, cluster PP's only, use PP-PP mask; **** Min PPDB Score 1.5, cluster PP's only, use PP-PP mask}; ***** Min PPDB Score 2.0, use PP-PP, Syn-Syn masks only; ****** Min PPDB Score 2.0, use PP-PP mask only}
  \label{tab:GAP1}
\end{table*}
...
%We use an oracle experiment to find the maximum GAP score achievable by adding a large number (10000) to the ranking model's score for words \textit{belonging to a single sense}. 
...
%using the paraphrase-only clustering method with generic embeddings, the paraphrase-only method with retrofitted embeddings, and the co-clustering method in Table \ref{tab:GAP1}. 
...
Finally, we find that for all methods, applying a PPDBScore Threshold is an effective way of removing noisy, non-substitutable paraphrases from the lexical substitution rankings. This supports the finding of \newcite{apidianaki:2016:EMNLP2016}, who showed that the PPDB2.0 Score itself is an effective lexical substitution ranking metric when large and noisy paraphrase substitute sets are involved.
